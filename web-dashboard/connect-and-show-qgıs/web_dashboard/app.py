...
def _db_connect():
    return psycopg2.connect(
        ...
        dbname=os.getenv("POSTGIS_DB", "wildfire_db"),
...
def stats():
    try:
            ...
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    SELECT
                        ...
                        COUNT(*) FILTER (WHERE prediction = 'fire') AS fire_frames,
                        COUNT(*) FILTER (WHERE prediction = 'no_fire') AS no_fire_frames,
                        COALESCE(MAX(fire_probability), 0) AS max_probability
                    FROM drone_frame_points;
                    """
                )
                total, fire_count, no_fire_count, max_prob = cur.fetchone()
        ...
        return jsonify({"status": "error", "message": f"PostGIS stats error: {exc}"})

    if total == 0 or min_lat is None or min_lon is None:
        return jsonify({
            "status": "empty",
            "center": [39.0, 35.0],
