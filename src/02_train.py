# Train RGB+thermal gated dual-branch fusion. Run from project root: python src/02_train.py
import argparse
...
def main():
    ap = argparse.ArgumentParser(description="Train gated dual-branch RGB+thermal fusion classifier")
    ap.add_argument("--csv", default=None, help="Index CSV or Parquet (default: master parquet if exists else flame CSV)")
    ap.add_argument("--mode", choices=["fusion"], default="fusion")
    ap.add_argument(
        "--model_family",
        choices=["dual_branch_gated_fusion"],
        default="dual_branch_gated_fusion",
        help="Only supported architecture (kept for CLI compatibility with scripts).",
    )
    ...
    fd = FUSION_TRAIN_DEFAULT

    if args.backbone is not None:
        backbone_resolved = str(args.backbone)
    else:
        backbone_resolved = str(fd.get("dual_branch_backbone", "resnet50"))

    resolved = {
        "loss_mode": args.loss_mode or fd.get("loss_mode", "sampler_focal"),
        "loss_name": (
            args.loss_name if args.loss_name is not None else fd.get("loss_name", "cb_focal")
        ),
        "lr": float(args.lr) if args.lr is not None else float(fd.get("lr", TRAIN_DEFAULT.get("lr", 1e-4))),
        "patience": int(args.patience)
        if args.patience is not None
        else int(fd.get("patience", TRAIN_DEFAULT.get("patience", 4))),
        "size": int(args.size) if args.size is not None else int(fd.get("size", TRAIN_DEFAULT.get("size", 384))),
        "backbone": backbone_resolved,
        "focal_gamma": float(fd.get("focal_gamma", TRAIN_DEFAULT.get("focal_gamma", 2.0))),
    }

    mf = args.model_family
    out = str(CKPT_DUAL_BRANCH)
    print(
        f"[02_train] mode=fusion family={mf} backbone={resolved['backbone']} "
