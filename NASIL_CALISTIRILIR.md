...
## 8. Realistic robustness değerlendirmesi (offline)

Eğitilmiş checkpoint için **tek protokol**: giriş tensörüne **çok hafif Gaussian blur severity=1** (düşük sigma, küçük çekirdek; yalnızca bu eval forward’unda). Çoklu bozunma grid’i veya clean satırı yoktur.
...
Çıktı: `outputs/robustness_eval.csv` — tek satır (`gaussian_blur`, sev=1) için metrikler.

> Bu modül yalnızca çıkarım ölçümüdür; eğitim augmentasyonlarını veya threshold seçimini değiştirmez. Üretim çıkarımı bu modüle bağlı değildir. Modality / dal ablation için `ablation_eval` kullanın.
